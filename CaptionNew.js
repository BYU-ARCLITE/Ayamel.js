(function(Ayamel){
	...
	Caption.FromCue = function(wrapper,processor,cue){
		...
		*/
		alr = cue.text.search(/[\u200F\u0590–\u07BF\uFB50–\uFDFF\uFE70–\uFEFF]/); //AL & R characters
		direction = (alr===0)?'rtl':'ltr';
		/*
		...
		*/
		...
		function lineOffset(top,bottom){
			var pos,unit;
			if(cue.rawLine === 'auto'){ style[bottom] = 0; }
			else{
				if(cue.snap){
					pos = cue.rawLine*getComputedStyle().lineHeight;
					unit = "px";
				}else{
					pos = cue.rawLine;
					unit = "%";
				}				
				if(pos<0){ style[bottom] = -1*pos+unit; }
				else{ style[top] = pos+unit; }
			}
		}
	};
